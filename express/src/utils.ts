...
export async function formatProfile(
  ...
  const avatarRecord = profile.records?.texts?.find((record) => record.key === "avatar")?.value;
...
async function encodeAvatar(name: string, avatarRecord: string | undefined): Promise<string | null> {
